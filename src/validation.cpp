...
// UniversalMolecule legacy difficulty-trend subsidy support — ported verbatim
// from universalmol-0.15.21 src/validation.cpp. The 0.8/15.21 chain pays a
// reward that depends on whether the previous block's difficulty is trending
// up or down. The "bork" range (heights 3600-4000) uses a target-based
// comparison; outside that range uses an average-difficulty comparison.
namespace {

    static const int64_t LEGACY_REWARD_LAUNCH_HEIGHT     = 1440;
    static const int64_t LEGACY_REWARD_SAMPLE_SPACING    = 20;
    static const int64_t LEGACY_REWARD_SAMPLE_COUNT      = 3;
    static const int64_t LEGACY_REWARD_BORK_HEIGHT       = 3600;
    static const int64_t LEGACY_REWARD_BORK_END_HEIGHT   = 4000;

    const CBlockIndex* LegacyRewardGetAncestor(const CBlockIndex* tip, int64_t target_height)
    {
        while (tip != nullptr && tip->nHeight > target_height) {
            tip = tip->pprev;
        }
        return tip != nullptr && tip->nHeight == target_height ? tip : nullptr;
    }

    arith_uint256 LegacyRewardTargetFromBits(unsigned int nBits)
    {
    ...
    double LegacyRewardGetDifficulty(const CBlockIndex* blockindex)
    {
        if (blockindex == nullptr) return 1.0;
    ...
    bool LegacyRewardDifficultyTrendIsRising(const CBlockIndex* pindexPrev)
    {
        if (pindexPrev == nullptr) return false;

        const int64_t baseHeight = pindexPrev->nHeight + 1;
        if (baseHeight < LEGACY_REWARD_BORK_HEIGHT) return false;

        int64_t sampleCount = 0;
        arith_uint256 cumulativeTarget;
        double cumulativeDifficulty = 0.0;

        for (int64_t i = 1; i <= LEGACY_REWARD_SAMPLE_COUNT; ++i) {
            const int64_t sampleHeight = baseHeight - (i * LEGACY_REWARD_SAMPLE_SPACING);
            if (sampleHeight <= 0) continue;

            const CBlockIndex* sample = LegacyRewardGetAncestor(pindexPrev, sampleHeight);
            if (sample == nullptr) continue;

            ++sampleCount;
            cumulativeTarget += LegacyRewardTargetFromBits(sample->nBits);
            cumulativeDifficulty += LegacyRewardGetDifficulty(sample);
        }

        if (sampleCount == 0) return false;

        const arith_uint256 baseTarget = LegacyRewardTargetFromBits(pindexPrev->nBits);
        if (baseHeight < LEGACY_REWARD_BORK_END_HEIGHT) {
            return baseTarget > (cumulativeTarget / sampleCount);
        }

        const double averageDifficulty = cumulativeDifficulty / sampleCount;
        return LegacyRewardGetDifficulty(pindexPrev) > averageDifficulty;
    }
...
{
    // UniversalMolecule subsidy: difficulty-trend rule (legacy 0.8). Genesis is
    // 1 UMO; pre-launch (height <= 1440) pays 0.001 UMO; thereafter difficulty
    // trend on the previous block decides between 0.1 UMO (rising) or 2 UMO.
    // nSubsidyHalvingInterval is intentionally unused.
    (void)consensusParams;
    if (nHeight == 0) {
        return COIN;
    }
    if (nHeight <= LEGACY_REWARD_LAUNCH_HEIGHT) {
        return COIN / 1000;
    }
